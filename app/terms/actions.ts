...
function buildCalendarDays(
  ...
  for (
    ...
    let dayType: "NORMAL" | "WEEKLY_OFF" | "HOLIDAY" | "SCHOOL_EVENT" = "NORMAL"
...
function buildWeekdaySlotCounts(rules: Array<{ weekday: number; defaultSlotCount: number }>) {
...
export async function createTerm(
  ...
  for (const weekday of [1, 2, 3, 4, 5]) {
    const value = Number(defaultSlotCountRaw[weekday])
