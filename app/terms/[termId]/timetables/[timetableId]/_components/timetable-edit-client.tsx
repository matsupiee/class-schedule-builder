...
export function TimetableEditClient({
  ...
  const handleAutoGenerate = async () => {
    if (!confirm("現在の時間割を自動生成で上書きしますか？")) {
      return;
    }
  ...
  return (
    <div className="grid gap-6">
      ...
      {requiredLessonCounts.length > 0 ? (
        <Card>
          <CardHeader>
            <CardTitle>授業消化数と法定必要数の比較</CardTitle>
            <CardDescription>
              この時間割プランについて、科目ごとの授業消化数と法定必要数を比較します。
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">消化数</TableHead>
                    <TableHead className="text-right">差分</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {requiredLessonCounts.map((rlc) => {
                    const actualCount = subjectSlotCounts.get(rlc.subjectId) ?? 0;
